...
static u64 read_msr(u32 msr)
{
	...
	printk(KERN_MSG "MSR 0x%08x => 0x%016llx\n", msr, result);
	return result;
}
...
static u16 read_vcore_hex(void)
{
	...
	printk(KERN_MSG "VCore hex offset => 0x%03x = %d\n", result, signed_result);
...
static void write_vcore_hex(u16 hex_offset)
{
	...
	printk(KERN_MSG "VCache hex offset <= 0x%03x = %d\n", hex_offset, signed_offset);
...
{
	printk(KERN_MSG "Removing module\n");
}
module_exit(mod_exit);

MODULE_AUTHOR("Jivan Pal - jivanpal.com");
MODULE_DESCRIPTION("Demonstrates use of MSRs in kernel driver");
MODULE_LICENSE("GPL");
