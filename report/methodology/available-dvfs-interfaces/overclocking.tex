...
\begin{itemize}
    ...
    \item \code{powersave} — despite having the same name as the
        \code{powersave} governor of \code{acpi-cpufreq}, this governor behaves
        like the \code{schedutil} governor of \code{acpi-cpufreq}, choosing a
...
The \code{CPUFreq} subsystem exposes an interface in SysFS that allows us to set
the current P-state as desired. We use the \code{cpupower} utility available in
...
reading from and writing to files in \code{/sys/} as appropriate.
